...
module.exports = class Repository {
  ...
  updaterepo() {
    ...
    return this.github.repos.update(this.settings).then((updatedRepo) => {
      this.log(`Here with security settings ${JSON.stringify(this.security)}`)   
  ...
  updateSecurity(parms, repoData){
    if (this.security) {
      ...
      if (enableVulnerabilityAlerts in repoData.data.security) {
          this.log(`Found enableVulnerabilityAlerts with security settings ${JSON.stringify(this.security)}`) 
          
          
          if(repoData.data.security.enableVulnerabilityAlerts.toLowerCase() == "true"){
            this.log(`Enabling Dependabot alerts for ${repoData.data.name}`)   
          }
          if(repoData.data.repoData.data.security,enableVulnerabilityAlerts.toLowerCase() == "false"){
            this.log(`Disabling Dependabot alerts for ${repoData.data.name}`)
          }
          else{
            this.log(`Can't find dependabot alert setting`)
          }
      } else {

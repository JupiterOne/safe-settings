...
module.exports = class Repository {
  ...
  updateSecurity(parms, repoData){
    if (this.security) {
      ...
      /*
      if (repoData.data?.security.length !== this.security.length ||
        !repoData.data?.security.every(t => this.security.includes(t))) {
          this.log(`Updating repo with security settings ${JSON.stringify(this.security)}`)   
          if(this.security.enableVulnerabilityAlerts === "True") {
            this.log(`enableVulnerabilityAlerts is True`)
          }
      } else {

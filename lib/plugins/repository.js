...
module.exports = class Repository {
    ...
    if (this.codeowners) {
      if (this.codeowners == true){
        if(await this.doesCodeOwnersExist(repoData)){
          ...
          //Check for PRs that this bot created
          const currentPR = await this.getOpenCodeownersPR(repoData)
          this.log(`Current PR: ${currentPR}`)
          if(!currentPR)
          {
            //If no PRs, create CodeOwners File
            //Find a codeowner
            const newCodeOwner = await this.findCodeOwner(repoData)
            this.log(`Current Codeowners: ${newCodeOwner}`)
            //Create a codeowners file and a PR to the main branch
          }
          return
        }
  ...
  async getOpenCodeownersPR(repoData) {
    let pull
    const res = await this.github.pulls.list({owner: repoData.data.owner.login, repo: repoData.data.name});
    for(pull of res.data){
      if(pull.title === "Safe-Settings Adding CODEOWNERS")
      //Codeowners PR exists. Return true
      return true
    }     
    return false
  }
  ...
  async findCodeOwner(repoData){
    ...
    //Check to see who created the repo
    try
    {
      const res = await this.github.activity.listRepoEvents({owner: repoData.data.owner.login, repo: repoData.data.name});
      ...
      //If no CreateEvent exists, it might be an old repo. Check for who made the most commits
      //const commits = (get commit list and order desc. grab the top)
    }
    catch (error)
    {
      this.log(`Error checking for CreateEvent ${error}`)
    }
  }
  ...
  async getCodeOwnersContent(repoData, path){
    ...
    const response = await this.github.repos.getContent(codeOwnersPayload).catch(e => {
      this.log(`Error getting codeownersfile ${JSON.stringify(codeOwnersPayload)} ${e}`);
      return false;
    })

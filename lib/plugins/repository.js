...
module.exports = class Repository {
  ...
  updateSecurity(parms, repoData){
    if (this.security) {
      ...
      this.log(`Found repo with security settings ${JSON.stringify(this.security.enableVulnerabilityAlerts)}`) 

      if (enableVulnerabilityAlerts in this.security) {
          this.log(`Found enableVulnerabilityAlerts with security settings ${JSON.stringify(this.security)}`) 
          
          
          if(this.security.enableVulnerabilityAlerts.toLowerCase() == "true"){
            this.log(`Enabling Dependabot alerts for ${repoData.data.name}`)   
          }
          if(this.security.enableVulnerabilityAlerts.toLowerCase() == "false"){
            this.log(`Disabling Dependabot alerts for ${repoData.data.name}`)
          }
          else{
            this.log(`Can't find enableVulnerabilityAlerts setting`)
          }
      } else {

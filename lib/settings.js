...
*/
  ...
  //CG Added visibility configs
  async loadConfigs() {
    this.repoConfigs = await this.getRepoConfigs(this.github, this.repo, this.log)
    this.visibilityConfigs = await this.getVisibilityConfigs(this.github, this.repo, this.log)
  ...
  async updateRepos(repo) {
    ...
    this.visibilityConfigs = this.visibilityConfigs || await this.getVisibilityConfigs(this.github, repo, this.log)
  ...
  async loadConfigMap(params) {
    try {
      ...
      if (Array.isArray(response.data)) {
        ...
        //CG Debug
        this.log.debug(` In loadConfigMap data ${JSON.stringify(response.data)}`)
        ...
        //CG Debug
        this.log.debug(` In loadConfigMap overrides ${JSON.stringify(overrides)}`)
      ...
      //CG Debug
      const  debugyaml = yaml.load(Buffer.from(response.data.content, 'base64').toString()) || {}
      this.log.debug(` In loadConfigMap debugyaml ${JSON.stringify(debugyaml)}`)
  ...
  async getRepoConfigMap() {
    try {
      ...
      //CG Debug
      this.log.debug(`In getRepoConfigMap params ${JSON.stringify(params)} `)
      this.log.debug(`In getRepoConfigMap reponse ${JSON.stringify(response)} `)
  ...
  async getVisibilityConfigMap() {
    try {
      this.log.debug(` In getVisibilityConfigMap ${JSON.stringify(this.repo)}`)
      const repo = { owner: this.repo.owner, repo: 'admin' }
      const CONFIG_PATH = '.github'
      const params = Object.assign(repo, { path: path.posix.join(CONFIG_PATH, 'visibility'), ref: this.ref })
      const response = await this.loadConfigMap(params)
   ...
   * @return The parsed YAML file
   */
     async getVisibilityConfigs() {
      try {
        const overridePaths = await this.getVisibilityConfigMap()
        ...
        this.log.debug(`visibility configs = ${JSON.stringify(visibilityConfigs)}`)
